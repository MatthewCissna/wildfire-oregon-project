...
def main() -> int:
    ...
    # Per-region predicted fire counts for the most recent season.
    try:
        ...
        merged = region_geom.merge(latest[["region", "pred_count", "pred_lo", "pred_hi"]], on="region")
        merged = gpd.GeoDataFrame(merged, geometry="geometry", crs=grid.crs)
        cmap_html = count_choropleth(merged, cfg.path_for("maps") / "fire_count_map.html")
        merged.drop(columns="geometry").to_csv(cfg.path_for("metrics") / "region_count_predictions.csv", index=False)
        print("✅ Fire-count map")
